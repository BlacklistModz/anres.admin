<div id="mainContainer" class="clearfix" data-plugins="main">
	<div role="main">
		<div class="span12">
			...
			<div class="uiBoxWhite pam clearfix">
				<div class="rfloat">
					<label class="label fwb" for="status">Status : 
						<select class="inputtext" name="status" data-plugins="_update" data-options="<?=$this->fn->stringify(array('url' => URL. 'registration/setData/'.$this->item['uid'].'/status'))?>">
							<?php 
							foreach ($this->status as $key => $value) {
								$sel = '';
								if( $this->item['status'] == $value['id'] ) $sel = ' selected="1"';
					...
					<tr>
						<td class="tac" width="33.33%">
							<ul>
								<li>
									<span class="gbtn">
										<button class="btn btn-red btn-no-padding"><i class="icon-remove"></i></button>
									</span>
								</li>
