...
class Media_Model extends Model
{
    ...
    private $_table = "media m 
        ...
        LEFT JOIN users emp ON m.media_emp_id=emp.user_id";
    ...
    public function resize($id, $fdata, $folder='', $minimize=array(500, 500)) {
    	...
    	if( !empty($data) ){
            ...
            $data['album_id'] = !empty($data['album_id']) ? $data['album_id'] : 0;

    		$filename = "{$folder}{$aid}_{$mid}_";
    		$original = WWW_UPLOADS."{$data['album_id']}".DS.$filename."o.{$type}";

    		if( file_exists($original) ){

    			$upload = new Upload();

    			$dest = WWW_UPLOADS."{$data['album_id']}".DS.$filename."n.{$type}";
